      ...
      <div className="container page-shell" style={{ padding: '32px 24px', maxWidth: '800px' }}>
        ...
        <form onSubmit={handleAnalyze} style={{ background: '#FFFFFF', border: '1px solid var(--border)', borderRadius: '16px', padding: '32px', marginBottom: '24px', boxShadow: '0 4px 24px rgba(0,0,0,0.06)' }}>
          ...
          <div style={{ marginBottom: '24px' }}>
            ...
            <textarea value={jobDesc} onChange={e => setJobDesc(e.target.value)} placeholder="Paste the job description here…" style={{ width: '100%', minHeight: '120px', padding: '12px 16px', fontSize: '16px', border: '1.5px solid var(--border)', borderRadius: '10px', background: '#FFFFFF', color: 'var(--text)', fontFamily: 'var(--font-body)', outline: 'none', transition: 'border-color 0.2s, box-shadow 0.2s', resize: 'vertical' }} onFocus={e => { e.target.style.borderColor = 'var(--primary)'; e.target.style.boxShadow = '0 0 0 3px rgba(59,130,246,0.12)' }} onBlur={e => { e.target.style.borderColor = 'var(--border)'; e.target.style.boxShadow = 'none' }} />
          </div>
